...
test("Detect Python .coverage", function()
  assert(utils.detect_format(".coverage") == "python_coverage")
end)
test("Detect Python coverage JSON", function()
  local tmp = "/tmp/crazy-coverage-python-json-" .. tostring(os.time()) .. ".json"
  local file = io.open(tmp, "w")
  assert(file)
  file:write([=[{"meta":{"format":3},"files":{"src/main.py":{"executed_lines":[1],"missing_lines":[2]}}}]=])
  file:close()
  assert(utils.detect_format(tmp) == "python_coverage")
  os.remove(tmp)
end)
...
test("Parse Python coverage JSON fixture", function()
  local data = parser.parse(cwd .. "/test/fixtures/sample_coverage_python.json")
  assert(type(data) == 'table')
