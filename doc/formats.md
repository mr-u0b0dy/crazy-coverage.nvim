...
crazy-coverage.nvim supports 8 coverage formats:

- **LCOV** (.info, .lcov) - Text format from gcov
- **LLVM JSON** (.json) - JSON export from llvm-cov
- **Python Coverage.py JSON** (.json) - JSON export from coverage.py
...
- Line coverage from `executed_lines` and `missing_lines`
- Branch coverage from `executed_branches` and `missing_branches`
- Native `.coverage` databases are auto-converted through the `coverage` CLI
...
- Compatible with coverage.py XML output (`coverage xml`)
